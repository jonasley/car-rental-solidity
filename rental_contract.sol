...
import {CarList} from "./car_list.sol";
...
contract Rental {
    ...
    CarList.Car storage carToBeRented = rentals[carId];
    ...
    function getRentalCarInfo(uint carId) public view returns (string, string, address, address, bool, uint, uint) {
      ...
      //Get specified car 
      CarList.Car memory specificCar = rentals[carId];
    ...
    function addNewCar(string make, address owner, string licenseNumber, uint year) public returns (uint) {
        ...
        CarList.Car memory newCar = CarList.Car(make,true, 0x0 , owner, year,licenseNumber,count);
    ...
    function returnCar(uint carId) public  returns (bool) {
        ...
        //Get Specific car
        CarList.Car storage specificCar = rentals[carId];
